...
- **Implementació:** endpoints/extensió Directus (`POST /auth/request-otp`, `POST /auth/verify-otp`); emmagatzematge de codis en col·lecció `auth_otps` (hash + expiració) o Redis; emissió de token/sessió Directus. Els usuaris de rol `partner` no necessiten contrasenya; els rols POLSER interns mantenen contrasenya + 2FA.
